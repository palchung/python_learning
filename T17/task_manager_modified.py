...
# convert a items into a dictionary
...
def generate_task_overview(task_file, task_overview_file):
...
def generate_user_overview(user_file, user_overview_file, task_list):
    ...
    total_user_incompleted_overdued = 0
...
# edit a task
def edit_task(task, completed, task_file, username_password):

    task_to_search = f"{task['username']};{task['title']};{task['description']};{task['due_date'].strftime(DATETIME_STRING_FORMAT)};{task['assigned_date'].strftime(DATETIME_STRING_FORMAT)};No"

    # if mark as completed for a task
    if completed:
        replacement_task = f"{task['username']};{task['title']};{task['description']};{task['due_date'].strftime(DATETIME_STRING_FORMAT)};{task['assigned_date'].strftime(DATETIME_STRING_FORMAT)};Yes"
...
while logged_in:
    ...
    if menu == 'r':
        ...
        # Always get tasks from file to make the data consistant.
        task_list = get_all_tasks_from(task_file)
        view_all(task_list)
        ...
        task_list = generate_task_overview(task_file, task_overview_file)
